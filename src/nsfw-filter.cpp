...
static void worker_main(nsfw_filter *f)
{
	...
	std::vector<uint8_t> buf(static_cast<size_t>(kInputSize) * kInputSize * 4);
	...
	for (;;) {
		...
		float infer_ms = 0.0f;
		try {
			if (!clf->Run(buf.data(), kInputSize * 4, probs, infer_ms))
				continue;
		} catch (const std::exception &e) {
		...
		f->has_result = true;
		f->res_frame = frame;
		std::memcpy(f->res_probs, probs, sizeof(probs));
...
static void *filter_create(obs_data_t *settings, obs_source_t *source)
{
	...
	f->self = source;
	f->job_buf.resize(static_cast<size_t>(kInputSize) * kInputSize * 4);
...
static void consume_result(nsfw_filter *f)
{
	...
	float lat_ms, infer_ms;
	{
		...
		std::memcpy(probs, f->res_probs, sizeof(probs));
	}

	f->lat_ms_ema = 0.9f * f->lat_ms_ema + 0.1f * lat_ms;

	const float score = std::min(1.0f, probs[kClassPorn] + probs[kClassHentai] +
						   f->sexy_weight.load() * probs[kClassSexy]);
	f->last_score = score;
	f->dbg_max_score = std::max(f->dbg_max_score, score);

	const uint64_t now = os_gettime_ns();
	if (score >= f->threshold.load()) {
		f->streak++;
		if (f->streak >= f->consecutive.load()) {
			if (!f->mask_active) {
				...
				blog(LOG_INFO, LOG_PREFIX "contenu sensible détecté (score %.2f) : masque activé", score);
			}
			f->last_pos_ns = now;
		}
	} else {
	...
	if (f->debug) {
		if (now - f->dbg_last_ns > 1000000000ULL) {
			blog(LOG_INFO,
			     LOG_PREFIX "score max=%.2f  dernier=%.2f | inférence=%.2f ms  latence=%.2f ms | délai=%d img | masque=%s",
			     f->dbg_max_score, score, infer_ms, f->lat_ms_ema, f->cur_delay,
			     f->mask_active ? "OUI" : "non");
			f->dbg_last_ns = now;
...
static void ensure_gfx(nsfw_filter *f, uint32_t w, uint32_t h, int delay)
{
	...
	for (auto &s : f->stage)
		if (!s)
			s = gs_stagesurface_create(kInputSize, kInputSize, GS_RGBA);
}
...
// Réduit l'image en 224x224, la rapatrie côté CPU (avec une image de retard,
// via deux surfaces en alternance pour ne jamais bloquer le GPU) et la
// soumet au worker.
static void analyze(nsfw_filter *f, gs_texture_t *full)
{
	...
	gs_ortho(0.0f, static_cast<float>(sz), 0.0f, static_cast<float>(sz), -100.0f, 100.0f);
	...
	gs_stage_texture(f->stage[cur], gs_texrender_get_texture(f->small_tr));
	f->stage_valid[cur] = true;
	f->stage_frame[cur] = f->frame_id;

	if (f->stage_valid[prev]) {
		uint8_t *data = nullptr;
		uint32_t linesize = 0;
		if (gs_stagesurface_map(f->stage[prev], &data, &linesize)) {
			{
				std::lock_guard<std::mutex> lk(f->job_m);
				for (uint32_t y = 0; y < sz; y++)
					std::memcpy(&f->job_buf[static_cast<size_t>(y) * sz * 4],
			...
			f->job_cv.notify_one();
			gs_stagesurface_unmap(f->stage[prev]);
		}
